    ...
    return (
        <div className='flex bg-gray-200 my-3 justify-between text-md'>
            <div
                ...
                className={`${
                    color === 0 ? defaultColor : 'bg-gray-200'
                } p-2 font-semibold text-center rounded w-full cursor-pointer`}>
                Popular
            </div>
            <div
                onClick={() => switchColor(1)}
                className={`${
                    color === 1 ? defaultColor : 'bg-gray-200'
                } p-2 font-semibold text-center w-full cursor-pointer`}>
                Recent
            </div>
            <div
                onClick={() => switchColor(2)}
                className={`${
                    color === 2 ? defaultColor : 'bg-gray-200'
                } p-2 font-semibold text-center rounded w-full cursor-pointer`}>
                Unanswered
            </div>
        </div>
